...
train_victor_corpus = sys.argv[8].strip()

def getMaxes(train_victor_corpus):
	map = {'0':0, '1':0, '2':0}
	f = open(train_victor_corpus)
	for line in f:
	...
	for key in map:
		sum += map[key]
	maxbad = 2.0*float(map['0'])/sum
	maxok = maxbad + 2.0*float(map['1'])/sum
	maxgood = 2.0
	return maxbad, maxok, maxgood
...
maxbad, maxok, maxgood = getMaxes(train_victor_corpus)
print('Maxes: ' + str([maxbad, maxok, maxgood]))
...
for Xte in Xtes:
	...
	if final_label<maxbad:
		final_label = 0
	elif final_label<maxok:
		final_label = 1
	else:
		final_label = 2
