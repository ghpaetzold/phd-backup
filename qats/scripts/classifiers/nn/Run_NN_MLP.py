...
try:
	...
	model = Sequential()
	model.add(Dense(HIDDEN_SIZE, input_dim=Xtr.shape[1], init="glorot_uniform"))
	model.add(Activation("tanh"))
	for i in range(0, LAYERS):
	        model.add(Dense(HIDDEN_SIZE, init="glorot_uniform"))
	        model.add(Activation("tanh"))
...
print('Training...')
if TRAIN:
	model.fit(Xtr, Ytr, nb_epoch=1000, batch_size=Xtr.shape[0])
