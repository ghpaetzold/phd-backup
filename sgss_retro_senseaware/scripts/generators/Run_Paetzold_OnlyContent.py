...
w2v = '/export/data/ghpaetzold/word2vecvectors/models/word_vectors_all_generalized_'+size+'_cbow_onlycontent.bin'

folderExists = os.path.isdir('../../substitutions/paetzoldonlycontent'+size)

if not folderExists:
	os.system('mkdir ../../substitutions/paetzoldonlycontent'+size)
	...
	out = open('../../substitutions/paetzoldonlycontent'+size+'/substitutions.txt', 'w')
	for k in subs.keys():
		newline = k + '\t'
		if len(subs[k])>0:
			for c in subs[k]:
