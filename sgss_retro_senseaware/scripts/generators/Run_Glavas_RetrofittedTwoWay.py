...
w2v = '/export/data/ghpaetzold/word2vecvectors/models/word_vectors_all_'+size+'_cbow_retrofittedtwoway.bin'

folderExists = os.path.isdir('../../substitutions/glavas_retrofittedtwoway'+size)

if not folderExists:
        kg = GlavasGenerator(w2v)
        subs = kg.getSubstitutions(victor_corpus, 10)

        os.system('mkdir ../../substitutions/glavas_retrofittedtwoway'+size)
        out = open('../../substitutions/glavas_retrofittedtwoway'+size+'/substitutions.txt', 'w')
        for k in subs.keys():
                newline = k + '\t'
                if len(subs[k])>0:
                        for c in subs[k]:
