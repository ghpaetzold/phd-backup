...
w2v = '/export/data/ghpaetzold/word2vecvectors/models/word_vectors_all_generalized_'+size+'_cbow.bin'

folderExists = os.path.isdir('../../substitutions/paetzold'+size)

if not folderExists:
	os.system('mkdir ../../substitutions/paetzold'+size)
	...
	out = open('../../substitutions/paetzold'+size+'/substitutions.txt', 'w')
	for k in subs.keys():
		newline = k + '\t'
		if len(subs[k])>0:
			for c in subs[k]:
