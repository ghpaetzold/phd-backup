...
loss = sys.argv[6].strip()
penalty = sys.argv[7].strip()
alpha = float(sys.argv[8].strip())
l1_ratio = float(sys.argv[9].strip())
epsilon = float(sys.argv[10].strip())
temp_file = sys.argv[11].strip()
proportion = float(sys.argv[12].strip())
k = sys.argv[13].strip()
if k!='all':
	k = int(sys.argv[13].strip())
...
bs.trainSelectorWithCrossValidation(victor_corpus, positive_range, folds, test_size, k=k)
