...
methods = ['kauchak']
lexf = open('../../../lexmturk/corpora/lexmturk_all.txt')
lex = []
for line in lexf:
...
for index in range(0, len(methods)):
	...
	for selector in sorted(selectors.keys()):
		if selector in maxims:
			...
			for file in selectors[selector]:		
				...
				pot, prec, rec, fmean = se.evaluateSelector('../../../lexmturk/corpora/lexmturk_all.txt', sele_d)
				if fmean>maxfmean:
					maxfmean = fmean
					maxpot = pot
					maxprec = prec
	...
	pot, prec, rec, fmean = ge.evaluateGenerator('../../../lexmturk/corpora/lexmturk_all.txt', orig_s)
	components = [pot, prec, rec, fmean]
	myt += r'\hline' + '\nNo Selection '
	for comp in components:
		cstr = "%.3f" % comp
