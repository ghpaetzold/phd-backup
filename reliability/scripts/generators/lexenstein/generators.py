...
class KauchakGenerator:
	...
	def getSubstitutions(self, victor_corpus):
		...
		subs = self.getInitialSet(victor_corpus, target_pos)
		substitutions_initial = copy.copy(subs)

		for word in sorted(subs.keys()):
			initials = set([])
			for pos in subs[word]:
				for cand in subs[word][pos]:
					initials.add(cand)
			print(word + ': ' + str(initials))

		print('\n')

		#Get final substitutions:
		print('Inflecting substitutions...')
		substitutions_inflected = self.getInflectedSet(subs)

		for word in sorted(substitutions_inflected.keys()):
			finals = set(substitutions_inflected[word])
			print(word + ': ' + str(initials))
