...
class FeatureEstimator:
		...
		for i in range(0, len(data)):
			...
			for subst in line[3:len(line)]:
				...
				for word in words.split(' '):
					cand_size += 1
					try:
						similarity += model.similarity(target+'|||'+self.getClass(target_pos), word+'|||'+self.getClass(target_pos))
					except KeyError:
						try:
							similarity += model.similarity(target+'|||'+self.getClass(target_pos), word.lower()+'|||'+self.getClass(target_pos))
						except KeyError:
							pass
	...
	def addWordVectorSimilarityFeature(self, model, java_path, pos_model, stanford_tagger, orientation):
		"""
		...
		if orientation not in ['Complexity', 'Simplicity']:
			...
			if model not in self.resources.keys():
				m = gensim.models.word2vec.Word2Vec.load_word2vec_format(model, binary=True)
				self.resources[model] = m
			self.features.append((self.wordVectorSimilarityFeature, [model, pos_model]))
	...
	def addWordVectorSimilarityNormalFeature(self, model, orientation):
		"""
		Adds a word vector similarity feature to the estimator.
		The value will be the similarity between the word vector of a target complex word and the word vector of a candidate.
		...
		if orientation not in ['Complexity', 'Simplicity']:
			...
			if model not in self.resources.keys():
				m = gensim.models.word2vec.Word2Vec.load_word2vec_format(model, binary=True)
				self.resources[model] = m
			self.features.append((self.wordVectorSimilarityNormalFeature, [model]))
			self.identifiers.append(('Word Vector Similarity (Model: '+model+')', orientation))

	def wordVectorSimilarityNormalFeature(self, data, args):
		model = self.resources[args[0]]
		result = []
		for line in data:
			target = line[1].strip().lower()
