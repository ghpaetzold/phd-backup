...
class WordVectorSelector:
	...
	def getSentVec(self, sentence, head, stop_words, window, onlyInformative, keepTarget, onePerWord, pos_tokens):
		...
		for i in range(0, len(tokensr)):
			token = tokensr[i]
			tokens.append(token+'|||'+self.getClass(pos_tokens[i][1].strip()))
	...
	def getWordVec(self, candidate, tag):
		result = []
		try:
			result = self.model[candidate+'|||'+self.getClass(tag)]
		except KeyError:
			try:
				result = self.model[candidate.lower()+'|||'+self.getClass(tag)]
			except KeyError:
				result = result
		return result
