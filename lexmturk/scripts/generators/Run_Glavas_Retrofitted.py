...
w2v = '/export/data/ghpaetzold/word2vecvectors/models/word_vectors_all_1300_cbow_retrofitted.bin'

kg = GlavasGenerator(w2v)
subs = kg.getSubstitutions(victor_corpus, 10)

out = open('../../substitutions/glavasretrofitted/substitutions.txt', 'w')
for k in subs.keys():
	newline = k + '\t'
	if len(subs[k])>0:
		for c in subs[k]:
