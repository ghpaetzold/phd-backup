...
print('SG/SS...')
o = open('../../corpora/analysis/SGSS_cumulative.txt', 'w')
...
for identifier in methodmap:
	...
	for generator in selfilemap:
		for selector in selfilemap[generator]:
			genselmap[(identifier, generator, selector)] = []
			...
			for i in range(0, len(allcands)):
				...
				if gold_label=='1':
					if len(ainter)==0:
						error3a += 1
						genselmap[(identifier, generator, selector)].append('Error')
					elif len(sinter)==0:
						error3b += 1
						genselmap[(identifier, generator, selector)].append('Error')
					else:
						noerror += 1
						genselmap[(identifier, generator, selector)].append('Ok')
				else:
					#noerror += 1
					genselmap[(identifier, generator, selector)].append('Ignore')
			o.write(identifier + '\t' + generator + '\t' + selector + '\tOK='+str(noerror) + '\t3A='+str(error3a) + '\t3B='+str(error3b) + '\n')	
o.close()
	

print('SR...')
o = open('../../corpora/analysis/SR_cumulative.txt', 'w')
for identifier in methodmap:
	...
	for line in fr:
		...
		ranker = frdata[2]
		file = frdata[3]
		indicators = genselmap[(identifier, generator, selector)]
