...
gold_standard = '../../corpora/lexmturk_gold.txt'
lexf = open(gold_standard)
lex = []
for line in lexf:
...
maxims = set(['wordvector', 'biran', 'void', 'clusters', 'clusters2k', 'boundaryCV', 'svmboundary'])
...
for index in range(0, len(methods)):
	...
	for selector in sorted(selectors.keys()):
		if selector in maxims:
			...
			for file in selectors[selector]:		
				...
				pot, prec, rec, fmean = se.evaluateSelector(gold_standard, sele_d)
				if fmean>maxfmean:
					maxfmean = fmean
					maxpot = pot
					maxprec = prec
	...
	pot, prec, rec, fmean = ge.evaluateGenerator(gold_standard, orig_s)
	components = [pot, prec, rec, fmean]
	myt += r'\hline' + '\nNo Selection '
	for comp in components:
		cstr = "%.3f" % comp
