...
methods = ['all']
lexf = open('../../corpora/lexmturk_gold.txt')
lex = []
for line in lexf:
...
gold_counts = []
gc = open('../../corpora/lexmturk_gold_counts.txt')
for line in gc:
	gold_counts.append(int(line.strip()))
gc.close()

selectors = getSelectors()
maxims = set(['wordvector', 'biran', 'void', 'postag', 'clusters', 'clusters2k', 'boundary', 'boundaryCV', 'svmrank'])
...
for index in range(0, len(methods)):
	...
	for selector in sorted(selectors.keys()):
		if selector in maxims:
			...
			for file in selectors[selector]:		
				...
				pot, prec, rec, fmean = se.evaluateSelector('../../corpora/lexmturk_gold.txt', sele_d)
				if selector=='void':
					print('VOID!')
	...
	pot, prec, rec, fmean = ge.evaluateGenerator('../../corpora/lexmturk_gold.txt', orig_s)
	components = [pot, prec, rec, fmean]
	myt += r'\hline' + '\nNo Selection '
	for comp in components:
		cstr = "%.3f" % comp
