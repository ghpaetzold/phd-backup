...
class MachineLearningIdentifier:
	...
	def trainSVM(self, C=1.0, kernel='rbf', degree=3, gamma=0.0, coef0=0.0, class_weight={0:1.0, 1:1.0}):
		...
		"""
		self.classifier = svm.SVC(C=C, kernel=kernel, degree=degree, gamma=gamma, coef0=coef0, class_weight=class_weight, probability=True)
...
class ThresholdIdentifier:
	...
	def trainIdentifierBruteForce(self, feature_index, step=None):
		...
		if not bestIndex:
			...
			print('max: ' + str(max))
			print('step: ' + str(step))
			print('steps: ' + str(len(myscores)))
			print('scores: ' + str(myscores))
	...
	def identifyComplexWords(self):
		...
		@return: A list of binary values, one per line, with value 1 if a target word is complex, and 0 otherwise.
		"""
		print('AT IDENTIFY: ' + str(self.threshold))
		result = []
	...
	def getConfidenceScores(self):
		mi = float(self.minX)
		ma = float(self.maxX)
		result = []
                for i in range(0, len(self.Xte)):
                        x = self.Xte[i][self.feature_index]
			divisor = abs(x-self.threshold)
                        dividend = 1
                        if x>self.threshold:
                                dividend = abs(ma-self.threshold)
                        else:
                                dividend = abs(mi-self.threshold)
