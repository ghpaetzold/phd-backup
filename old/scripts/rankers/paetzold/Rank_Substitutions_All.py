...
#arg1 = kbest features
#arg2 = outfile
#arg3 = maximize precision accuracy changed
...
def calculateFeatures(data):
    ...
    feature_vector = [alignprobs, colocSubimdb]
...
Xtr = readFeatures('./features/features_training_best.txt')
Ytr = generateLabels(raw_training_features, positive_range)

#Create feature selector:
feature_selector = SelectKBest(f_classif, k=int(sys.argv[1]))
					...
					for r in l1ratios:
						for e in epsilons:
							...
							try:
								...
								precision, accuracy, changed = calculateKauchak(raw_testing_features, Xte, clf, training_subs)
								scores_obtained = [precision, accuracy, changed]
								if scores_obtained[maximize_index]>maxv:
									maxv = scores_obtained[maximize_index]
									maxvalues = [precision, accuracy, changed]
							except ValueError:
								aux = 0
		
		table.append([prefix, maxvalues[0], maxvalues[1], maxvalues[2]])
	outf.write(tabulate(table))
	outf.write('')
outf.close()
