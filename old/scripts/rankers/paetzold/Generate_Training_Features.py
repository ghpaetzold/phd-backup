...
def calculateFeatures(data):
    ...
    lmWikipedia = '../../../../machinelearningranking/corpora/lm/simplewiki.5gram.bin.txt'
    colocSubtlex = getCollocationalFeatures(data, lmSubtlex, 4, 4)
    colocSubimdb = getCollocationalFeatures(data, lmSubimdb, 2, 2)
    colocWikipedia = getCollocationalFeatures(data, lmWikipedia, 2, 2)
    ...
    feature_vector = [lengths, syllables, senses, synonyms, hypernyms, hyponyms, mindepths, maxdepths]
    feature_vector.extend([colocSubimdb])
    #feature_vector.extend([colocSubtlex, colocSubimdb, colocWikipedia])
    feature_vector.extend([docCountsWikipedia, docCountsSimpleWikipedia, docCountsSubimdb])
    #feature_vector = [colocSubimdb]
...
ftrainout = open('training_features.txt', 'w')
