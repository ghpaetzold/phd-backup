...
def calculateKauchak(datate, X, clf, subs, modelw, modelsw):
    total = 0
    totalc = 0
    precise = 0
	...
	line = datate[i]
        ...
        for subst in line[3:len(line)]:
            ...
            tuples.append((subst, getBiran(word, modelw, modelsw)))
            index += 1
        tuples = sorted(tuples, reverse=True)
        tuples = sorted(tuples, key=lambda item: item[1], reverse=False)
...
def getBiran(word, modelw, modelsw):
	freqc = modelw.score(word, bos=False, eos=False)
	freqs = modelsw.score(word, bos=False, eos=False)
	return float(len(word))*freqc/freqs
...
modelw = kenlm.LanguageModel('../../../corpora/lm/wiki.5.bin.txt')
modelsw = kenlm.LanguageModel('../../../corpora/lm/simplewiki.5.bin.txt')
...
for method in methods:
	...
	for prefix in prefixes:
		...
		precision, accuracy, changed = calculateKauchak(raw_testing_features, [], None, training_subs, modelw, modelsw)
